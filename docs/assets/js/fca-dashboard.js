  ...
  function initFirmsTable(){
    const tbl = $('#firms-table').DataTable({
      ...
      columns: [
        ...
        { data: d => getNamesByFrn(d.frn).length, title:'#Names' },
        { data: d => (normalizeARAppointments()[d.frn] || []).length, title:'#ARs' },
  ...
  function renderFirmDetails(d){
    ...
    function renderTable(title, cols, data){
      ...
      const hdr  = cols.map(c=>`<th>${c}</th>`).join(''),
            body = data.map(r=>`<tr>${cols.map(c=>`<td>${r[c]!=null?r[c]:''}</td>`).join('')}</tr>`).join('');
    ...
    return `<div class="child-rows">
      ...
      ${renderTable('Appointed Reps',
        ['Name','Principal FRN','Principal Firm Name','Effective Date','EEA Tied Agent','Tied Agent','[NotinUse] Insurance Distribution'], ars)}
  ...
  function initIndividualsTable(){
    ...
    const tbl = $('#individuals-table').DataTable({
      ...
      columns: [
        ...
        { data: d => Object.entries(indivData||{}).filter(([_,arr])=>arr.some(e=>e.IRN===d.irn)).map(([frn])=>frn).join(', '), title:'Firms' },
        { data: d => {
            let cnt=0, nm=d.name;
            Object.values(cfData||{}).forEach(arr=>arr.forEach(c=>{ if(c['Individual Name']===nm) cnt++; }));
            return cnt;
          }, title:'#CF Records' }
      ],
      order: [[1,'asc']]
    ...
    $('#individuals-table tbody').on('click','td:nth-child(2)', function(){
      ...
      if (row.child.isShown()){
        ...
        const d = row.data(), cfEntries=[];
        Object.values(cfData||{}).forEach(arr=>arr.forEach(c=>{ if(c['Individual Name']===d.name) cfEntries.push(c); }));
        const details = cfEntries.length
          ? `<table class="child-table"><thead><tr><th>Section</th><th>Function</th><th>Effective Date</th></tr></thead>
               <tbody>${cfEntries.map(c=>`<tr><td>${c.section}</td><td>${c.controlled_function}</td><td>${c['Effective Date']}</td></tr>`).join('')}</tbody>
             </table>`
  ...
  function initARsTable(){
    ...
    const tbl = $('#ars-table').DataTable({
      ...
      columns: [
        ...
        { data: d=>d.insurDist?'✓':'✗', title:'Insur. Dist.', className:'dt-center' }
      ],
      order: [[1,'asc']]
    });
    ...
    $('#ars-table tbody').on('click','td:nth-child(2)', function(){
      ...
      if (row.child.isShown()){
        ...
        const d    = row.data(), recs = apptMap[d.arFrn]||[],
              hdr  = '<th>Principal FRN</th><th>Principal Name</th><th>Eff. Date</th><th>EEA-Tied</th><th>Tied-Agent</th>',
              body = recs.map(r=>`<tr><td>${r['Principal FRN']||''}</td><td>${r['Principal Firm Name']||''}</td><td>${r['Effective Date']||''}</td><td>${r['EEA Tied Agent']||''}</td><td>${r['Tied Agent']||''}</td></tr>`).join('');
